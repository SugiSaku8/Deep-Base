  ...
  _initVersionCommands() {
    ...
    this.version = {
      ...
      compare: (version1, version2) => {
        ...
        this.log({from: 'db.sys.version', message: `Version comparison: ${version1} vs ${version2}`, level: 'info'});
...
Examples:
  ...
  // ログ出力API
  log({from = 'db.sys.unknown', message = '', level = 'info', timestamp = null}) {
  ...
  showApp(appName) {
    this.log({from: `db.app.${appName}.out`, message: `${appName}アプリを表示中`, level: 'info'});
    this.currentApp = appName;
    this.log({from: `db.app.${appName}.out`, message: `現在のアプリ: ${appName}`, level: 'info'});
  }
  ...
  loadApp(appName) {
    ...
    if (!appRoot) {
      ...
      this.log({from: 'db.sys.shell', message: '#app-root がなかったので自動生成しました', level: 'warn'});
    }
    ...
    if (appModule && typeof appModule.appInit === 'function') {
      ...
      this.log({from: `db.app.${appName}.err`, message: `アプリ${appName}の初期化関数が見つかりません`, level: 'warn'});
    }
  }
}
